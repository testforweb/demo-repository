<template>
  <div><canvas id="mycanvas_tan" width="600" height="500"></canvas></div>
</template>
...
export default {
  mounted() {
    ...
    function draw() {
      var canvas = document.getElementById('mycanvas_tan')
      if (canvas.getContext) {
        ...
        function drawOther() {
          ...
          for (let i = 0; i < bgTicks + 1; i++) {
            let y =
              Math.ceil(
                padding.top +
                  ((height - padding.bottom - padding.top) / bgTicks) * i
              ) + 0.5
            new Text(
              domain[1] - (domain[1] / bgTicks) * i,
              padding.left - 6,
              y,
              'end',
              'middle'
          ...
          for (let i = 0; i < dataset.x.length + 1; i++) {
            let x =
              Math.ceil(
                padding.left +
                  ((width - padding.left - padding.right) / dataset.x.length) *
                    i
              ) + 0.5
        ...
        var startX =
            padding.left +
            (width - padding.left - padding.right) / dataset.x.length / 2,
        ...
        canvas.onmousemove = function (event) {
          ...
          for (let i = 0; i < dataset.y.length; i++) {
            let x =
              padding.left +
              (width - padding.left - padding.right) / dataset.x.length / 2 +
              ((width - padding.left - padding.right) / dataset.x.length) * i
        ...
        function yScale(value) {
          let scale =
            Math.abs(domain[1] - domain[0]) / Math.abs(range[0] - range[1])
        ...
        function seriesLabel() {
          ...
          min =
            min < 0
              ? Math.floor(
                  Math.floor(min) /
                    Math.pow(10, Math.floor(min).toString().length - 1)
                ) * Math.pow(10, Math.floor(min).toString().length - 1)
              : 0
          ...
          max =
            max == 0
              ...
              : Math.ceil(
                  Math.ceil(max) /
                    Math.pow(10, Math.ceil(max).toString().length - 1)
                ) * Math.pow(10, Math.ceil(max).toString().length - 1)
          return [min, max]
        }
        // 一次性定义整条曲线
